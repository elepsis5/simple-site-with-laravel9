<section class="page-section bg-primary text-white mb-0" id="about">
    <div class="container">
        <!-- About Section Heading-->
        <h2 class="page-section-heading text-center text-uppercase text-white">About</h2>
        <!-- Icon Divider-->
        <div class="divider-custom divider-light">
            <div class="divider-custom-line"></div>
            <div class="divider-custom-icon"><i class="fas fa-star"></i></div>
            <div class="divider-custom-line"></div>
        </div>
        <!-- About Section Content-->
        <div class="row">
            <div class="col-lg-6 ms-auto">
                <p class="lead">
                    <i class="bi bi-caret-right-fill"> Мини-сайт с интерфейсом отображения статей.</i><br>
                    <i class="bi bi-caret-right-fill"> Страница статьи разбита на компоненты с помощью Vue3 + Vuex4.</i><br>
                    <i class="bi bi-caret-right-fill"> Реализована инкрементация лайков и просмотров.</i><br>
                    <i class="bi bi-caret-right-fill"> Реализована возможность добавления коментариев.</i><br>
                </p>
            </div>
            <div class="col-lg-6 me-auto">
                <p class="lead">
                    <i class="bi bi-caret-right-fill"> Отдельная страница с выводом всех статей с пагинацией.</i><br>
                    <i class="bi bi-caret-right-fill"> Наполнение тестовыми данными с использование фабрик и сидеров.</i><br>
                    <i class="bi bi-caret-right-fill"> Вся логика реализована на основе стандартных инструментов Laravel.</i><br>
                    <i class="bi bi-caret-right-fill"> Стилизация на основе Bootstrap v5.1.</i><br>
                </p>
            </div>
        </div>
        <!-- About Section Button-->
        <div class="text-center mt-4">
            <a class="btn btn-xl btn-outline-light" href="https://github.com/elepsis5/simple-site-with-laravel9.git">
                <i class="bi bi-github"></i>
                Go to GitHub!
            </a>
        </div>
    </div>
</section>
